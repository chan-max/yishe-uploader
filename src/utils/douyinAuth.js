...
export class DouyinAuth {
    constructor() {
        this.authDataPath = join(process.cwd(), 'auth-data', 'douyin-auth.txt');
        this.cookies = [];
        this.headers = {};
    }
    ...
    parseAuthData() {
        try {
            if (!existsSync(this.authDataPath)) {
                logger.warn('抖音认证数据文件不存在:', this.authDataPath);
                return null;
            }
    ...
    async setCookies(page) {
        try {
            ...
            const douyinCookies = setCookies.filter(c =>
                c.domain.includes('douyin.com')
            );

            logger.info(`实际设置了${douyinCookies.length}个抖音Cookie`);

            return true;
        } catch (error) {
            logger.error('设置抖音认证Cookie失败:', error);
            return false;
        }
    }
    ...
    getRequestHeaders() {
        ...
        return {
            ...
            'Origin': authData.headers.origin || 'https://creator.douyin.com',
            'Referer': authData.headers.referer || 'https://creator.douyin.com/',
    ...
    async applyAuth(page) {
        try {
            ...
            if (success) {
                logger.info('抖音认证应用成功');
            }
            return success;
        } catch (error) {
            logger.error('应用抖音认证失败:', error);
            return false;
        }
    }
}
...
// 创建单例实例
export const douyinAuth = new DouyinAuth();
