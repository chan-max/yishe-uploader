...
import { douyinAuth } from '../utils/douyinAuth.js';
...
class DouyinPublisher {
    ...
    async publish(publishInfo) {
        ...
        try {
            ...
            // 3. 应用认证
            const authSuccess = await douyinAuth.applyAuth(page);
            if (!authSuccess) {
                throw new Error('抖音认证设置失败');
            }
            logger.info('抖音认证已应用');
    ...
    async fillTitleAndTags(page, publishInfo) {
        ...
        try {
            // 方式1: 通过相对定位
            const titleContainer = page.locator('text=作品标题')
                .locator('..')
                .locator('xpath=following-sibling::div[1]')
                .locator('input');
            
            const count = await titleContainer.count();
            if (count > 0) {
                await titleContainer.fill(publishInfo.title.substring(0, 30));
                logger.info('标题填写成功（方式1）');
            } else {
                throw new Error('方式1失败，尝试方式2');
            }
        } catch (error) {
            // 方式2: 通过 .notranslate 类
            logger.info('尝试方式2填写标题');
            const titleContainer = page.locator('.notranslate').first();
        ...
        if (publishInfo.tags && publishInfo.tags.length > 0) {
            ...
            for (let i = 0; i < publishInfo.tags.length; i++) {
                ...
                await page.locator(cssSelector).type(`#${tag}`);
                await page.locator(cssSelector).press('Space');
                logger.info(`已添加话题: #${tag}`);
            }
            logger.info(`总共添加了 ${publishInfo.tags.length} 个话题`);
        }
    }
