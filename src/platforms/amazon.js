...
class AmazonPublisher {
    constructor() {
        this.platformName = '亚马逊';
        this.uploadUrl = 'https://sellercentral.amazon.com/products/create';
        ...
        try {
            ...
            if (publishInfo.title) {
                await this.pageOperator.fillInput(
                    page,
                    'input[name*="product-title"], input[aria-label*="title"], input.title-input',
                    publishInfo.title
                );
                logger.info('标题填充完成');
            }
            ...
            if (publishInfo.description) {
                await this.pageOperator.fillInput(
                    page,
                    'textarea[name*="feature"], textarea[aria-label*="description"]',
            ...
            // 7. 点击保存/发布按钮
            const submitSelector = 'button[name*="save"], button[aria-label*="save"], button:has-text("Save"), button:has-text("Publish")';
    ...
    async _checkLogin(page) {
        try {
            const loginElements = await page.$$('.login-btn, .login-button, .auth-btn, .sign-in');
...
export const amazonPublisher = new AmazonPublisher();
