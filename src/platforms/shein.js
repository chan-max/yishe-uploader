...
class SheinPublisher {
    constructor() {
        this.platformName = '希音';
        this.uploadUrl = 'https://seller.shein.com/seller-center/product/products';
    ...
    async publish(publishInfo) {
        ...
        try {
            ...
            if (publishInfo.description) {
                await this.pageOperator.fillInput(
                    page,
                    'textarea[placeholder*="商品描述"], textarea[placeholder*="说明"], textarea.product-desc',
            ...
            // 6. 点击发布/保存按钮
            const submitSelector = 'button[type="primary"], button.btn-primary, button:has-text("发布"), button:has-text("保存")';
            await this.pageOperator.clickButton(page, submitSelector);
            logger.info('已点击发布/保存按钮');
    ...
    async _checkLogin(page) {
        try {
            const loginElements = await page.$$('.login-btn, .login-button, .login-entry, .auth-btn');
...
export const sheinPublisher = new SheinPublisher();
