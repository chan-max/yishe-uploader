...
class XianyuPublisher {
        ...
        try {
            ...
            if (publishInfo.title) {
                await this.pageOperator.fillInput(
                    page,
                    'input[placeholder*="商品名称"], input[placeholder*="标题"]',
                    publishInfo.title
                );
                logger.info('标题填充完成');
            }
            ...
            if (publishInfo.description) {
                await this.pageOperator.fillInput(
                    page,
                    'textarea[placeholder*="商品描述"], textarea[placeholder*="说明"]',
            ...
            // 6. 点击发布按钮
            const submitSelector = 'button[type="primary"], button.next-btn-primary, button:has-text("发布")';
    ...
    async _checkLogin(page) {
        try {
            const loginElements = await page.$$('.login-btn, .login-button, .login-entry, .login-text');
